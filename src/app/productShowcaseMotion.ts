// Motion tuning for the "Overview of FYT" (ProductShowcase) premium polish
// pass — kept out of App.tsx, same rationale/convention as
// proveSkillReveal.ts / proofInNumbersMotion.ts.
import type { Transition, Variants } from "motion/react";

const EASE_OUT_EXPO = [0.16, 1, 0.3, 1] as const;
const SPRING: Transition = { type: "spring", stiffness: 260, damping: 22 };

// ─── Section-level reveal order: heading → dashboard → Trusted Platform
// card → Trusted Support card. These 4 are siblings in that exact DOM
// order already (see ProductShowcase in App.tsx), so one orchestrating
// container with staggerChildren delivers the requested order for free.
...
// ─── Heading: word-by-word (not a hard line-break split — the heading is a
// single natural sentence with no fixed line count across breakpoints;
// splitting on words preserves the exact existing responsive wrapping
// instead of forcing a layout change).
...
// ─── Feature badge row (3 items: "Secure by design" / "Real-time updates" /
// "Built for traders") — 80ms stagger, icon pops in via spring slightly
// before its label fades in.
...
// ─── Dashboard "assembly": a back copy of the same image (no second asset
// exists — see App.tsx comment) settles into a fixed slight tilt behind the
// front copy, which then rises/scales/fades in on top.
...
// Idle float + mouse parallax (parallax uses the existing useTilt(5) hook —
// see App.tsx) for the assembled dashboard group.
...
// Reflection sweep every 12-15s (not continuous).
...
// ─── 5-8 tiny floating blue dust particles (not stars — soft/blurred, not
// crisp points) scattered across the section, each on its own slow drift.
...
// ─── Trusted Support Team card hover ────────────────────────────────────
export const productSupportCardHover = { y: -4 } as const;

// Pulsing "online" indicator dots (Live Chat / Always On icons).
...
// Chat button: resting appearance is unchanged (still a flat white pill —
// see App.tsx) — this is a slow, very-low-opacity blue sheen that sweeps

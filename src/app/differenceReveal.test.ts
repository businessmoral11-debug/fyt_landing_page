...
describe("DIFFERENCE_PIN_SCROLL_HEIGHT_VH", () => {
  ...
  // Guards the 150→195 recalibration (see the code comment above this
  // constant): 195vh total minus the 100vh sticky viewport leaves ~95vh of
  // extra scroll runway, matching the reference site's measured pinned
  // section far more closely than the old 150vh value (~50vh of runway,
  // which made the crossfade feel rushed). A tolerance band around that
  // ~95vh target, rather than an exact-95 check, so this fails loudly on an
  // accidental revert to 150 (40vh of runway) or an unrelated regression,
  // without being so tight it breaks on a deliberate small retune.

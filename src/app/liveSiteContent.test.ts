...
describe("LIVE_PAYOUTS_CONTENT / LIVE_PAYOUTS_BACKEND", () => {
  it("matches verified live copy and the real backend's connection details", () => {
    expect(LIVE_PAYOUTS_CONTENT).toEqual({
      heading: "Live payouts. Verified rewards.",
      tickerLabel: "RECENT VERIFIED REWARDS",
    });
    expect(LIVE_PAYOUTS_BACKEND).toEqual({
...
describe("VIDEO_URLS", () => {
  it("has exactly 1 hero + 14 testimonial + 3 misc hotlinked URLs, all https on fundingyourtrades.com", () => {
    ...
    // No duplicate URLs across the whole set.

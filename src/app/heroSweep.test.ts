...
// A synthetic 6-angle fixture, deliberately NOT imported from heroOrbit.ts —
// this file tests heroSweep.ts's generic pacing/keyframe math, which must
// stay correct regardless of how many labels the hero currently has or what
// their real angles are. Coupling it to HERO_ORBIT_LABELS meant an unrelated
// content edit (e.g. changing the label count) would silently break these
// assertions; see docs/superpowers/plans/2026-07-26-hero-headline-radar-labels.md.
...
describe("glowKeyframes", () => {
  ...
  it("throws on conflicting values at the same rounded stop", () => {
    // riseStart rounds to the same 3-decimal stop as holdStart but carries opacity 0 vs 1
...
describe("heroSweepCss", () => {
  ...
  it("negates stored bearings into beam angles (starts pointing LEFT at −58°)", () => {
    // ANGLES[0] is −58 → beam bearing +58 at the first dwell;
    // ANGLES[5] is +58 → beam bearing −58 at the last.
    expect(css).toContain("0% { transform: rotate(58deg); }");
    expect(css).toContain("100% { transform: rotate(-58deg); }");
    expect(css).toContain("10.256% { transform: rotate(58deg); animation-timing-function: ease-in-out; }");
  });

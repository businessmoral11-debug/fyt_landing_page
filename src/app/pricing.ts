// Live fundingyourtrades.com pricing data, product IDs, and resolution helpers.
// Values sourced verbatim from the site's PRODUCTS_CONFIG. Do not edit numbers by hand —
// run `node scripts/check-pricing-drift.mjs` (add --write to patch) to re-sync against
// the live site instead. priceNew reflects whatever discount the live site is currently
// running; it is NOT a fixed percentage of priceOld, since that promo rate changes over
// time (this is what caused this data to go stale before — see git history for the fix).
// Last verified against the live config: 2026-07-28.
...
// One row = one account size, with prices + IDs for both platforms.
// mt / p5 tuples are [priceOld, priceNew, productId]. `split` overrides the plan default for that row.
interface Row {
  size: number;
...
// ── Rule sets (constant within a step/plan) ─────────────────────────────────
...
// ── Data (product IDs + prices from live config) ────────────────────────────
export const PRICING_DATA: Record<StepId, Partial<Record<PlanId, PlatformTable>>> = {
  "1-Step": {
    ...
    // fyt-prime · 1-STEP (identical rules/prices; "no consistency rule")
  ...
  "2-Step": {
    ...
    // fyt-pro · 2-STEP-∞ (identical rules/prices; different product)
  ...
  "Instant": {
    ...
    // fyt-pro · INSTANT (prices identical across platforms; IDs differ)
...
// Display order matches the "Trusted Platform" showcase section on the page:
// MatchTrader first, Platform 5 second. Also the default `platform` state.
...
// Spec display labels (v3 uses words; ids stay numeric for data keys)
...
// v3 card line under the price. Classic refunds after the second reward,
// Prime/Plus after the first (kept consistent with the FAQ answer).
...
// Small badge on the plan-selector button in the pricing calculator.
// Only Prime removes the consistency-rule requirement; Classic/Plus keep it.

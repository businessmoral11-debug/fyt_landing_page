// Verified verbatim copy pulled from https://fundingyourtrades.com (FYT's own
// live production site). Every later phase imports from here instead of
// hardcoding strings, so a copy correction only has to happen once.

export const HERO_CONTENT = {
  headlineMain: "Trade without Hidden Rules.",
  headlineBlue: "Get Rewards in 24 Hours.",
  checklist: ["No Consistency Rule", "Up to 100% Reward Split", "1st Reward on Demand", "Static Drawdown"],
  ctaPrimary: { label: "Get Started Now", href: "#challenge" },
  ctaSecondary: { label: "Chat with us", href: "https://discord.gg/6NYNSAwxeZ" },
} as const;

export const PROMO_BANNER = {
  headingPrefix: "New To FYT? ",
  headingHighlight: "40% Off + Buy 1 Get 2 Instantly",
  body: "Receive 40% OFF + Instant BOGO using the code FYT40 for new users only. Don't miss this exclusive offer!",
  code: "FYT40",
  footnote: "Valid for new users only. Terms and conditions apply.",
} as const;

export const FAQ_ITEMS = [
  {
    q: "When does the Evaluation/Challenge Start?",
    a: "Your evaluation/challenge begins the moment your account credentials are sent to you. Get ready to trade from day one!",
  },
  {
    q: "Keep 18% of your challenge rewards!",
    a: "Starting from your second reward, you'll receive 18% of the virtual targets earned during the challenge plus your refundable fee!",
  },
  {
    q: "Do I have to pay the loss incurred on the account?",
    a: "When you sign up with us, you're choosing peace of mind. We take full responsibility for any losses incurred on your account, you are not liable for covering them.",
  },
  {
    q: "Is the Evaluation fee refundable?",
    a: "Your evaluation fee is 100% refundable after your second reward — our way of recognizing your commitment. Anything greater than 100% that you see are bonuses that comes with particular promos. Therefore, anything more will depend on the particular promo that you signed up with.",
  },
] as const;

export const KEY_METRICS = [
  { value: "$2.6M+", label: "In Rewards" },
  { value: "14,000+", label: "FYT Traders" },
  { value: "105+", label: "Countries" },
] as const;

export const LIVE_PAYOUTS_CONTENT = {
  heading: "Live payouts. Verified rewards.",
  tickerLabel: "RECENT VERIFIED REWARDS",
} as const;

// Real production backend for the live ticker (see Global Constraints in the
// Phase 1 plan). `.websocket` is consumed by useLiveRewardsFeed() in
// liveRewardsFeed.ts (Phase 6) to power the Recent Rewards table.
// `.configEndpoint` is still unused — this section keeps its own hardcoded
// heading/labels rather than the real widget's configurable ones.
export const LIVE_PAYOUTS_BACKEND = {
  websocket: "wss://devrite.fundingyourtrades.com/ws/certs",
  configEndpoint: "https://devrite.fundingyourtrades.com/api/embed-config",
} as const;

export const FEATURES_GRID = {
  heading: "What you get from us?",
  cards: [
    { title: "Get 18% Reward Bonus", desc: "Earn an extra in bonus rewards when you complete the challenge" },
    { title: "Up to 100% Reward Split", desc: "Earn the highest profit share from your performance" },
    { title: "Up to 200% Refund Bonus", desc: "Get rewarded even more with refund incentives" },
    { title: "Weekly Rewards & Free Reset", desc: "Faster payouts and second chances to boost your success" },
    { title: "No Time Limit", desc: "Trade at your own pace — no deadlines, no pressure" },
    { title: "Static Drawdowns", desc: "Fair and consistent drawdown rules based on balance" },
    { title: "Weekend Holding", desc: "Keep positions open overnight or over the weekend" },
    { title: "News Trading", desc: "Trade/hold positions without restrictions with our challenge accounts" },
  ],
} as const;

export const PAYOUT_STATISTICS = [
  { value: "Up to 100% Split", label: "Paid up to 100% reward splits" },
  { value: "105 Minutes", label: "Average Reward Speed" },
  { value: "$2.6M+", label: "Reward Paid Out" },
] as const;

export const VERIFICATION_SECTION = {
  kickerGold: "The Only Prop Firm",
  kickerWhite: " Giving Direct Access To Verified Paid Traders",
  headingMain: "Verify Us Before You Commit. Hear Straight From",
  headingHighlight: " Traders Who've Already Been Paid.",
  subheading:
    "In an industry full of doubt, we're the only prop firm that lets you speak directly with traders who've already been paid.",
  // NOTE: this href independently returned HTTP 404 during research — a
  // live-site bug, preserved as-is rather than silently corrected.
  cta: { label: "Speak with a Funded Trader", href: "https://fundingyourtrades.com/funding-your-trades-payouts/#speak" },
} as const;

export const HOW_IT_WORKS = {
  heading: "How You Can Get Simulated Funding",
  steps: [
    { label: "Challenge", desc: "Prove Your Skills Here" },
    { label: "Funded", desc: "Get Funded Up To 200K" },
    { label: "Rewards", desc: "Get Up To 100% Rewards" },
  ],
} as const;

export const PLATFORMS_SECTION = {
  heading: "available trading platform",
  subheading: "Access powerful platforms designed to maximize your trading rewards",
  platform5: {
    title: "Platform 5",
    body: "Unlock the full potential of your trading with Platform 5, engineered for elite traders who demand precision, speed and advanced features that deliver results, empowering you to stay ahead of every market move.",
  },
  matchTrader: {
    title: "Match-Trader",
    body: "MatchTrader is a fast-rising platform known for its intuitive user interface and powerful functionalities. Designed to streamline your trading experience",
  },
  testAccountHref: "https://intercom.help/funding-your-trades/en/articles/12850361-test-accounts",
} as const;

export const TRADER_FEEDBACK = {
  heading: "Fyt traders feedback and reviews",
  subheading:
    "Hear directly from the traders who choose FYT. From first rewards to consistent monthly withdrawals their words, their wins.",
  quotes: [
    {
      name: "Javier",
      quote:
        "Funding YourTrades feels more transparent now. No vague rules or hidden conditions. What's on the website is exactly what you get. That kind of honesty matters a lot in this space.",
    },
    {
      name: "Hafeez Ullah",
      quote:
        "Phase2 Passed, Alhamdolillah Phase02 25K with @fundingurtrades Profit Traget Completed. Over all good expereince with @fundingurtrades. Good trading conditions and crypto spreads are very low. Powerfull trading dashboard. Lets hope soon i will get Funded acount",
    },
    {
      name: "Sahirr A Haji",
      quote:
        "FYT is stand out of crowd due to no daily draw down limit, What gives me more space to manage equity accordingly... Match-trader platform is outstanding",
    },
    {
      name: "taxfreehogg",
      quote:
        "I have only been trading with FYT for a couple of weeks, but I love it so far. Credentials are sent out almost immediately after purchasing a challenge, as well as after passing a Phase 1 or 2 challenge. I'll update this once I have my first live account underway, but it's been a wonderful experience up until this point.",
    },
    {
      name: "Aeston",
      quote:
        "I had a payout rejected under the old FYT team last year, and honestly, I had written it off as a loss. But recently, I got an email from the new team saying they were reviewing old cases - and they refunded me! That kind of gesture shows real integrity. I'm giving FYT another shot because of this.",
    },
    {
      name: "Arthur",
      quote:
        "Passed the challenge and just received my first payout. It hit my account the next day - no excuses, no follow-ups needed. That reliability earns major points from me.",
    },
    {
      name: "Michael Brown",
      quote:
        "Started off with the $10K account just to test things out. Passed in 2 weeks. Now I'm funded and just received my second payout. Smooth experience so far.",
    },
    {
      name: "Finley Cartwright",
      quote:
        "I've been with FYT since the early days, and the changes after the ownership shift have been huge. The platform feels smoother, the rules make more sense, and the support is 10x better than before. You can tell the new team actually listens to traders.",
    },
  ],
} as const;

export const HEAR_FROM_TRADERS = {
  heading: "Hear From Our Traders",
  cta: { label: "Become Next Success Story!", href: "https://fundingyourtrades.com/#challenge" },
} as const;

export const SUPPORT_SECTION = {
  heading: "24x7 Around the Clock Support",
  subheading: "Always-on support team available for your trading journey.",
  cta: { label: "Get Support", href: "#support" },
} as const;

export const FINAL_CTA = {
  heading: "100X Your Rewards. Start Your Funded Journey Today",
  body: "Unlock bigger rewards and a smarter path to simulated trading. Our challenge gives you the opportunity and rules designed for long‑term success, while our community keeps you motivated, informed, and improving every day. When you are with us, you never trade alone and your rewards grow with your discipline",
} as const;

export const FOOTER_LINKS = {
  nav: [
    { label: "Rewards Proof", href: "https://fundingyourtrades.com/fundingyourtrades-reviews-trader-results/" },
    { label: "Affiliate Registration", href: "https://fundingyourtrades.com/affiliate-registration/" },
    { label: "Affiliate Login", href: "https://fundingyourtrades.com/affiliate-account/" },
    { label: "Affiliate Reset Password", href: "https://fundingyourtrades.com/affiliate-reset-password/" },
    { label: "FAQ", href: "https://intercom.help/funding-your-trades/en/" },
    { label: "Contact Us", href: "https://fundingyourtrades.com/contact/" },
    { label: "Terms of Services", href: "https://fundingyourtrades.com/terms-of-services/" },
    { label: "Privacy Policy", href: "https://fundingyourtrades.com/privacy-policy/" },
    { label: "Risk Disclosure", href: "https://fundingyourtrades.com/risk-disclosure/" },
    { label: "Anti-Money Laundering Policy", href: "https://fundingyourtrades.com/anti-money-laundering-policy/" },
    { label: "Cookie Policy", href: "https://fundingyourtrades.com/cookie-policy/" },
    { label: "Refund Policy", href: "https://fundingyourtrades.com/refund_returns/" },
  ],
  social: [
    { label: "Discord", href: "https://discord.gg/6NYNSAwxeZ" },
    { label: "Instagram", href: "https://www.instagram.com/fundingyourtrades" },
    { label: "Telegram", href: "https://t.me/fundingyourtrades" },
    { label: "X", href: "https://x.com/fundingurtrades" },
    { label: "YouTube", href: "https://www.youtube.com/@FundingYourTrades" },
  ],
} as const;

// Live nav structure (verified against https://fundingyourtrades.com's actual
// header). Replaces the old placeholder labels (Programs/Pricing/Trading
// Rules/Platforms) entirely — see the CTA link wiring plan.
export const NAV_LINKS = {
  items: [
    { type: "link", label: "Rewards Proof", href: "https://fundingyourtrades.com/fundingyourtrades-reviews-trader-results/" },
    {
      type: "dropdown",
      label: "Affiliate",
      items: [
        { label: "Affiliate Registration", href: "https://fundingyourtrades.com/affiliate-registration/" },
        { label: "Login", href: "https://fundingyourtrades.com/affiliate-account/" },
        { label: "Reset Password", href: "https://fundingyourtrades.com/affiliate-reset-password/" },
      ],
    },
    { type: "link", label: "FAQ", href: "https://intercom.help/funding-your-trades/en/" },
    { type: "link", label: "Contact Us", href: "https://fundingyourtrades.com/contact/" },
  ],
  login: { label: "Login/Sign up", href: "https://clients.fundingyourtrades.com/login" },
  buyHere: { label: "Buy Here", href: "#challenge" },
} as const;

// Footer nav columns. Distinct from FOOTER_LINKS.nav above (a flat 12-item
// list that isn't rendered anywhere) — this mirrors the footer's actual
// 3-column PRODUCT/COMPANY/LEGAL layout in App.tsx, mixing live-site URLs
// with this site's own in-page anchors (#how-it-works, #live-payouts).
export const FOOTER_COLUMNS = [
  {
    heading: "PRODUCT",
    items: [
      { label: "How it works", href: "#how-it-works" },
      { label: "Reward ledger", href: "#live-payouts" },
    ],
  },
  {
    heading: "COMPANY",
    items: [
      { label: "FAQ", href: "https://intercom.help/funding-your-trades/en/" },
      { label: "Affiliate", href: "https://fundingyourtrades.com/affiliate-registration/" },
      { label: "Contact", href: "https://fundingyourtrades.com/contact/" },
    ],
  },
  {
    heading: "LEGAL",
    items: [
      { label: "Terms of Service", href: "https://fundingyourtrades.com/terms-of-services/" },
      { label: "Privacy Policy", href: "https://fundingyourtrades.com/privacy-policy/" },
      { label: "Risk Disclosure", href: "https://fundingyourtrades.com/risk-disclosure/" },
      { label: "Refund Policy", href: "https://fundingyourtrades.com/refund_returns/" },
    ],
  },
] as const;

// All video assets are hotlinked to the live site rather than self-hosted —
// several exceed GitHub's 100MB blob limit, and re-encoding/hosting them
// ourselves was judged not worth the storage cost for this phase. These URLs
// were verified reachable (HTTP 200) during Phase 1 research. Risk: if FYT
// ever moves/removes these files on their live site, playback breaks here
// with no local fallback — accepted tradeoff per project decision.
export const VIDEO_URLS = {
  hero: {
    welcome: "https://fundingyourtrades.com/wp-content/uploads/2025/07/Welcome-to-FYT-2.mp4",
  },
  testimonials: {
    damien: "https://fundingyourtrades.com/wp-content/uploads/2025/12/Damien-trader.mp4",
    edmond: "https://fundingyourtrades.com/wp-content/uploads/2025/12/Edmond_2.mp4",
    gabriel: "https://fundingyourtrades.com/wp-content/uploads/2025/12/Gabriel-trader.mp4",
    waqas: "https://fundingyourtrades.com/wp-content/uploads/2025/12/Waqas-trader.mp4",
    rebecca: "https://fundingyourtrades.com/wp-content/uploads/2026/03/Rebecca-final-interview.mp4",
    anisha: "https://fundingyourtrades.com/wp-content/uploads/2026/05/Anisha-part-2.mp4",
    charles: "https://fundingyourtrades.com/wp-content/uploads/2026/05/Charles-part-2.mp4",
    dylan: "https://fundingyourtrades.com/wp-content/uploads/2026/05/FYT-Dylan.mp4",
    mayFirst: "https://fundingyourtrades.com/wp-content/uploads/2026/05/FYT-may-1st.mp4",
    maySecond: "https://fundingyourtrades.com/wp-content/uploads/2026/05/FYT-may-2nd-2nd-ver.mp4",
    shubam: "https://fundingyourtrades.com/wp-content/uploads/2026/05/Shubam-part-2.mp4",
    nadir: "https://fundingyourtrades.com/wp-content/uploads/2026/06/NADIR-HUSSAIN-FINAL-.mp4",
    nicholas: "https://fundingyourtrades.com/wp-content/uploads/2026/06/Nicholas-Video.mp4",
    rohit: "https://fundingyourtrades.com/wp-content/uploads/2026/06/ROHIT-VIDEO-FINAL-.mp4",
  },
  misc: {
    howToPurchase: "https://fundingyourtrades.com/wp-content/uploads/2025/05/How-to-make-a-Successful-Purchase-at-FYT.mp4",
    fyt7: "https://fundingyourtrades.com/wp-content/uploads/2026/01/FYT-7.mp4",
    fyt8: "https://fundingyourtrades.com/wp-content/uploads/2026/01/FYT-8.mp4",
  },
} as const;

// Portable public embed — no FYT-specific auth needed to reuse verbatim.
export const TRUSTINDEX_WIDGET = {
  loaderSrc: "https://cdn.trustindex.io/loader.js?1c0d261737201100820687d5af1",
  dataPid: "521672873e0c61161e165e3e45b",
  dataLayoutId: "29",
  dataSetId: "dark-minimal",
} as const;
